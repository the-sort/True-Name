...
# + odkomentovat test_get_first_test_mask test_get_second_test_mask
...
def linter():
    ...
    r = Run(
        ["--disable=C0301,C0103,E1101,R0913,R0914,R0917", "-sn", src_file],
        reporter=rep,
        exit=False,
    )
    return r.linter
...
#         np.savez_compressed(REF_PATH / f"{img_base}_{threshold}_{border}_get_first_test_mask.npz", mask_ref=mask)
...
#     assert mask[:, -border:].sum() == 0
#     if COMPUTE_REF_ARRAYS:
#         np.savez_compressed(REF_PATH / f"{img_base}_{threshold}_{border}_get_second_test_mask.npz", mask_ref=mask)
...
def test_calculate_kp_scores(input_image, threshold, border):
    ...
    second_test_mask = np.load(REF_PATH / f"{img_base}_{threshold}_{border}_get_second_test_mask.npz")['mask_ref']
    second_test_passed_r, second_test_passed_c = np.where(second_test_mask)

    if COMPUTE_REF_ARRAYS:
        indices = []
        druhy = set(zip(second_test_passed_r, second_test_passed_c))
        for i, k in enumerate(zip(first_test_passed_r, first_test_passed_c)):
            if k in druhy:
                # keypoints2.append(k)
                indices.append(i)
        np.savez_compressed(REF_PATH / f"{img_base}_{threshold}_{border}_get_second_test_mask_indices.npz", mask_ref=np.array(indices))

    second_test_mask_ind = np.load(REF_PATH / f"{img_base}_{threshold}_{border}_get_second_test_mask_indices.npz")['mask_ref']
    keypoints2 = list(
        zip(
            first_test_passed_r[second_test_mask_ind], first_test_passed_c[second_test_mask_ind]
        )
    )

    scores = orb.calculate_kp_scores(img, keypoints2)
    assert isinstance(scores, list)
    assert all([isinstance(score, int) for score in scores])
    if COMPUTE_REF_ARRAYS:
        np.savez_compressed(REF_PATH / f"{img_base}_{threshold}_{border}_calculate_kp_scores.npz", scores_ref=scores)
    scores_ref = (
...
@pytest.mark.parametrize(
    "threshold,border", [[20, 0], [20, 20], [20, 10], [15, 10], [15, 5]]
)
def test_detect_keypoints(input_image, threshold, border):
    ...
    if COMPUTE_REF_ARRAYS:
        np.savez_compressed(REF_PATH / f"{img_base}_{threshold}_{border}_detect_keypoints_1.npz", keypoints_ref=keypoints)
        np.savez_compressed(REF_PATH / f"{img_base}_{threshold}_{border}_detect_keypoints_2.npz", scores_ref=scores)
...
def test_get_x_derivative(input_image):
    ...
    if COMPUTE_REF_ARRAYS:
        np.savez_compressed(REF_PATH / f"{img_base}_get_x_derivative.npz", result_ref=result)
...
def test_get_y_derivative(input_image):
    ...
    assert result.shape == img.shape
    if COMPUTE_REF_ARRAYS:
        np.savez_compressed(REF_PATH / f"{img_base}_get_y_derivative.npz", result_ref=result)
...
def test_get_harris_response(input_image):
    ...
    if COMPUTE_REF_ARRAYS:
        np.savez_compressed(REF_PATH / f"{img_base}_get_harris_response.npz", response_ref=response)
...
def test_filter_keypoints(input_image, n_max, threshold, border):
    ...
    if COMPUTE_REF_ARRAYS:
        np.savez_compressed(REF_PATH / f"{img_base}_{n_max}_{threshold}_{border}_filter_keypoints.npz", filtered_keypoints_ref=filtered_keypoints)
    ...
    print("\n")
    for i, kpr in enumerate(filtered_keypoints_ref):
        print(kpr, '==', filtered_keypoints[i], ' -> ', kpr == filtered_keypoints[i])
