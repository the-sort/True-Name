...
class CMenu():
    ...
    def __init__(self, manager):
        ...
        self.__decline.move(
            (SCREEN_W//2 - self.__decline.width(),SCREEN_H - self.__decline.height() - off_set)
        )
        self.__accept.move(
            (SCREEN_W//2, SCREEN_H - self.__accept.height() - off_set)
        )
    ...
    def display(self, delta_time):
        ...
        if self.__quit: #Player pressed leave button
            ...
            self.__confirm.move(
                (SCREEN_W // 2 - self.__confirm.width(), question_top_left[1] + size[1])
            )
        ...
        if self.__wizzad.state(): #opened shop
            ...
            self.__screen.blit(
                self.__board,
                self.__center_align(
                    (self.__board.get_width(), self.__board.get_height())
                )
            )
