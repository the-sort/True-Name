...
class CShop:
    ...
    def display(self, screen, manager):
        """
        ...
        coins = font.render(str(manager.player.coins()), True, (255, 255, 255))
        font = pygame.font.Font("./dictionaries/Gothic_pixel_font_fixed.ttf", 17)
        health = font.render(str(manager.player.health())+"/1000", True, (255, 255, 255))
        prize_txt = font.render(str(prize), True, (255, 255, 255))
        ...
        prize_txt_size = prize_txt.get_rect().size
        ...
        small_coin_top_left = (health_top_right[0] - self.__plus.width() - off_set, health_top_right[1])
        prize_top_left = (small_coin_top_left[0] - prize_txt_size[0] - off_set // 5, small_coin_top_left[1])
        plus_top_left = (prize_top_left[0] - self.__plus.width() - off_set // 5, small_coin_top_left[1])

        self.__plus.move(plus_top_left)

        screen.blit(self.__shop_frame, shop_frame_top_left)
        screen.blit(coins, coins_am_top_left)
        screen.blit(self.__coin_img, (coins_am_top_left[0] + coins.get_width() + off_set, coins_am_top_left[1]))
        screen.blit(health, health_top_left)
        screen.blit(self.__health_img, health_img_top_left)
        screen.blit(prize_txt, prize_top_left)
        screen.blit(self.__small_coin, small_coin_top_left)
        
        self.__plus.display(screen, 0, 0)

    # def __render_font(self, font_size):
    #     """
